...
struct FathomClientTests {
    ...
    func parseDocumented() throws {
        ...
        {"items":[
          {"id":"rec_123","title":"Ambient Sync","created_at":"2026-06-30T17:00:00Z",
           ...
           "transcript":[
             {"speaker":"Alex","text":"Let's start.","timestamp":0},
             {"speaker":"Dom","text":"Agreed.","timestamp":"00:01:05"}
           ]}
        ],"next_cursor":"abc"}
        """
    ...
    func parseAlternateKeys() throws {
        ...
        {"meetings":[
          {"recording_id":"r9","meeting_title":"Quick chat","recording_start_time":1782800000,
           "transcript":[{"speaker":{"name":"Priya"},"content":"Hi","start":12.5}]}
        ]}
        """
        let (meetings, cursor) = try #require(FathomParse.meetings(from: data(json)))
        #expect(cursor == nil)
        ...
        #expect(meetings.first?.lines.first?.speaker == "Priya")
    ...
    func toParsed() {
        let m = FathomMeeting(id: "1", title: "Sync", createdAt: Date(timeIntervalSince1970: 1_782_800_000),
                              durationSeconds: 600,
                              lines: [.init(speaker: "Alex", text: "a", tStart: 0),
                                      .init(speaker: "Dom", text: "b", tStart: 5),
                                      .init(speaker: "Alex", text: "c", tStart: 9)],
                              summaryMarkdown: nil)
        let p = m.toParsedTranscript()
        #expect(p.source == .fathom)
        #expect(p.title == "Sync")
        #expect(p.speakers == ["Alex", "Dom"])     // distinct, first-seen order
        #expect(p.utterances.count == 3)
        #expect(p.utterances[1].speakerRaw == "Dom")
        // Line ends are derived from the next line's start; last from the meeting duration (D6).
        #expect(p.utterances[0].tEnd == 5)      // next line starts at 5 (was 0 == tStart)
        #expect(p.utterances[1].tEnd == 9)
        #expect(p.utterances[2].tEnd == 600)    // last → durationSeconds
        #expect(p.utterances.allSatisfy { $0.tEnd > $0.tStart })   // no zero-length utterances
    }
