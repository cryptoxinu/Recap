...
struct AIImporterTests {
    ...
    func detect() {
        ...
        Sounds good.
        """
        let json = #"{"sentences":[{"speaker_name":"Dom","text":"hi","start_time":1}]}"#
        let prose = "We talked about pricing and Riley said to come under OpenRouter. No structure here."
    ...
    func liveAIResolve() async throws {
        let messy = """
        ok so this was the call w/ dom and riley earlier — dom said we should price at 10% under openrouter,
        riley jumped in: "we just need to come under the open router pricing we're currently at". then we
        talked TEEs, dom explained encrypted-in-use ram. i asked about cost basis, he said ~225 per gpu hour,
        8 gpus a machine.
        """
