...
struct DriveOAuthTests {
    ...
    func fetchPlan() {
        ...
        // REGRESSION (founder 2026-07-01): a shared Google Meet recording is video/mp4 with NO extension in
        // its name ("Alex <> Team"). It must map by MIME to mp4, not be dropped for lacking a name-extension.
        #expect(plan("video/mp4", "Alex <> Team (2026-06-30)")?.ext == "mp4")
        #expect(plan("video/quicktime", "Weekly sync")?.ext == "mov")
        #expect(plan("audio/mp4", "Call audio")?.ext == "m4a")
        #expect(plan("video/x-matroska", "some recording")?.ext == "mp4")   // unknown video → mp4 container
    }
