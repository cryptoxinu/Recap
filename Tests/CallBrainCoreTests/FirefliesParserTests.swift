...
struct FirefliesParserTests {
    ...
    static let sample = """
    {
      "title": "Riley sync — Render GPU pricing",
      "date": 1747238400000,
      "duration": 52.0,
      "participants": ["riley@render.com", "me@company.com"],
      "sentences": [
        {"index": 0, "speaker_name": "Riley", "speaker_id": "0", "text": "On Render, the GPU spot pricing dropped this week.", "start_time": 12.0, "end_time": 18.5},
        {"index": 1, "speaker_name": "Me", "speaker_id": "1", "text": "Good — does that change the validator economics?", "start_time": 18.6, "end_time": 22.0},
        {"index": 2, "speaker_name": "Riley", "speaker_id": "0", "text": "Yeah, materially for inference hardware.", "start_time": 22.1, "end_time": 25.0}
      ]
    }
    """
    ...
    func parsesCore() throws {
        ...
        #expect(t.title == "Riley sync — Render GPU pricing")
        #expect(t.utterances.count == 3)
        #expect(t.speakers == ["Riley", "Me"])               // first-seen order
    ...
    func nestedTranscript() throws {
        ...
        {"transcript": {"title": "Nested", "sentences": [
          {"speaker_name": "Dom", "text": "Proof of Logits means…", "start_time": 1.0, "end_time": 4.0}
        ]}}
        """
        let t = try FirefliesParser.parse(Data(nested.utf8))
        #expect(t.title == "Nested")
        #expect(t.utterances.count == 1)
        #expect(t.utterances[0].speakerRaw == "Dom")
    }
}
