...
struct ConversationStoreTests {
    ...
    func roundTrip() throws {
        ...
        try store.appendMessage(Message(id: "m1", conversationID: "c1", role: .user,
                                        text: "What did Riley say about Render?", createdAt: 110))
        let cites = [StoredCitation(tag: "S1", chunkID: "ck1", meetingID: "mt1", speaker: "Riley", text: "Render pricing dropped.")]
        try store.appendMessage(Message(id: "m2", conversationID: "c1", role: .assistant,
                                        text: "Riley said pricing dropped [S1].", citations: cites, createdAt: 120))
    ...
    func deleteMeetingScrubs() throws {
        ...
        try store.saveMeeting(Meeting(id: "mt", title: "Sensitive call", date: "2026-06-29", source: .fireflies),
                              chunks: [Store.ChunkInput(chunkID: "mt_c0", meetingID: "mt", version: 0, seq: 0,
                                       speaker: "Dom", tStart: 0, tEnd: 1, text: "secret roadmap details", contentHash: "h")])
        ...
        try store.appendMessage(Message(id: "m2", conversationID: "cglob", role: .assistant, text: "Per the call [S1].",
                                        citations: [StoredCitation(tag: "S1", chunkID: "mt_c0", meetingID: "mt",
                                                                   speaker: "Dom", text: "secret roadmap details")], createdAt: 3))
...
/// v12 (perfection Task 3.1): reasoning steps + provider survive a Recents reload.
@Suite("Message steps/provider persistence (v12)")
struct MessageStepsPersistenceTests {
    @Test("steps and provider round-trip; legacy rows decode with empty steps")
    func testStepsProviderRoundTrip() throws {
        let path = FileManager.default.temporaryDirectory
            .appendingPathComponent("cb-v12-\(UUID().uuidString).sqlite").path
        let store = try Store(path: path)
        try store.upsertConversation(Conversation(id: "c1", title: "t", meetingID: nil, createdAt: 1, updatedAt: 1))
        let steps = [AskEngine.ReasoningStep(id: 0, icon: "brain", title: "Understanding", detail: "x"),
                     AskEngine.ReasoningStep(id: 1, icon: "magnifyingglass", title: "Searching", detail: "y")]
        try store.appendMessage(Message(id: "m1", conversationID: "c1", role: .assistant, text: "hi",
                                        createdAt: 2, steps: steps, provider: "claude"))
        let back = try store.messages(conversationID: "c1")
        #expect(back.first?.steps == steps)
        #expect(back.first?.provider == "claude")
        // Legacy-shaped row (no steps/provider) still decodes.
        try store.appendMessage(Message(id: "m2", conversationID: "c1", role: .user, text: "q", createdAt: 3))
        #expect(try store.messages(conversationID: "c1").last?.steps.isEmpty == true)
    }
}
