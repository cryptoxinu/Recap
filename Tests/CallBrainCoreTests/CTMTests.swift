...
struct CTMTests {
    ...
    func utteranceCodableRoundTrip() throws {
        let u = Utterance(
            ...
            personID: "p_travis", speakerRaw: "Riley", speakerConfidence: 0.88,
    ...
    func meetingCodableRoundTrip() throws {
        let m = Meeting(
            id: "m1", title: "Riley sync — Render GPU pricing", date: "2026-05-14",
            startedAt: Date(timeIntervalSince1970: 1_747_238_400), durationSeconds: 3120,
            source: .fathom, company: "Render",
            participants: [ParticipantRef(personID: "p_travis", rawLabel: "Riley", role: "speaker")],
    ...
    func citationDeepLink() {
        let c = Citation(
            chunkID: "c1", meetingID: "m1", meetingTitle: "Riley sync — Render",
            meetingDate: "2026-05-14", speaker: "Riley", tStart: 742.30, tEnd: 768.11,
    ...
    func citationNoTimestamp() {
        let c = Citation(
            ...
            speaker: "Dom", tStart: nil, tEnd: nil, source: .cluely, tsConfidence: .none
        )
    ...
    func sourceRawValues() {
        ...
        #expect(MeetingSource.gmeetCaptions.rawValue == "gmeet_captions")
        #expect(MeetingSource.srtVtt.rawValue == "srt_vtt")
        #expect(MeetingSource.allCases.count == 10)
    }
}
