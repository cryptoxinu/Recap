...
| 2026-06-29 | P1 | **IngestEngine** — parse→chunk→embed→store glue (`ingestFireflies`/`ingestFathom`); SHA-256 content hashing | Sources/CallBrainCore/Ingest/IngestEngine.swift · Tests/.../IngestEngineTests.swift | ✅ **40/40 green** | (Codex gate running) | **Headless MVP engine COMPLETE** — ingest + ask both wired. Remaining P1 = SwiftUI app (Xcode target) |
