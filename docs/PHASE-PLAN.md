...
| 2026-06-29 | P1 ∥B/∥D | **Hybrid retrieval core**: vector math (cosine, Float32 BLOB codec, brute-force topK) + RRF fusion + `OllamaEmbedder` + `SearchEngine` (FTS⊕vector→RRF, candidate pre-filter, vector-only hydration) + Store vector persistence | Sources/CallBrainCore/{Retrieve/Vector,Retrieve/RRF,Retrieve/SearchEngine,Embedding/Embedder}.swift + Store methods + 2 test files | ✅ **35/35 green** (offline StubEmbedder hybrid e2e) | (pending phase gate) | Single-model query/doc (D7); live ollama embed opt-in `CALLBRAIN_LIVE_OLLAMA=1` |
