...
public struct AIImporter: Sendable {
    ...
    /// When false (Settings "Local-only mode"), the importer NEVER calls the cloud LLM: an unrecognized
    /// format is kept on-device as a plain single-track transcript and a missing title falls back to a local
    /// default, so importing a call in local-only mode egresses nothing (audit F1).
    public let allowCloud: Bool
    public init(llm: any LLMProvider, allowCloud: Bool = true) { self.llm = llm; self.allowCloud = allowCloud }

    public enum Format: String, Sendable, Equatable {
        case firefliesJSON, firefliesCopy, fathom, geminiNotes, subtitle, unknown
    }
    ...
    private static let copyHeaderRE = try! NSRegularExpression(
    ...
    private static let fathomHeaderRE = try! NSRegularExpression(
        pattern: #"(?m)^\s*.{1,40}?(?:\s+\d{1,2}:\d{2}(?::\d{2})?\s*$|\(\d{1,2}:\d{2}(?::\d{2})?\)\s*:?.*$)"#)
    private static let subtitleCueRE = try! NSRegularExpression(
        pattern: #"(?m)^\s*(?:\d{1,2}:)?\d{1,2}:\d{2}(?:[,.]\d{1,3})?\s*-->\s*(?:\d{1,2}:)?\d{1,2}:\d{2}(?:[,.]\d{1,3})?(?:\s|$)"#)
    ...
    /// `.docx` with bold colon-less `Riley 0:00` lines was mis-routing to Gemini, and a single-section
    /// notes paste with incidental timecodes was being shredded as Fathom.)
    public static func detect(_ raw: String, fileExtension: String? = nil) -> Format {
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        let t = trimmed.hasPrefix("\u{FEFF}") ? String(trimmed.dropFirst()) : trimmed
        guard !t.isEmpty else { return .unknown }
        if isSubtitleExtension(fileExtension) { return .subtitle }
        let full = NSRange(location: 0, length: (t as NSString).length)
        if t.lowercased().hasPrefix("webvtt") || subtitleCueRE.firstMatch(in: t, range: full) != nil {
            return .subtitle
        }
        if t.hasPrefix("{") || t.hasPrefix("[") {
            // Route to FirefliesParser ONLY on its load-bearing structural markers — it requires a
            // `sentences[]` array. A bare `"transcript"` substring matched unrelated JSON (e.g. the
            // Fathom API shape `"transcript":[…]`) and shredded it in a parser that needs `sentences`
            // (audit D10). A non-Fireflies JSON now falls through to AI-resolve / needs-review.
            if t.contains("\"sentences\"") || t.contains("\"speaker_name\"") {
    ...
    private static func isSubtitleExtension(_ ext: String?) -> Bool {
        guard let ext else { return false }
        let normalized = ext.trimmingCharacters(in: CharacterSet(charactersIn: ".").union(.whitespacesAndNewlines))
            .lowercased()
        return normalized == "srt" || normalized == "vtt"
    }
    ...
    public func resolve(_ raw: String, generateTitleIfMissing: Bool = true,
                        titleHint: String? = nil, dateHint: String? = nil,
                        fileExtension: String? = nil) async throws -> Resolved {
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { throw ParseError.empty }

        switch Self.detect(trimmed, fileExtension: fileExtension) {
        case .firefliesJSON:
        ...
        case .geminiNotes:
            ...
            return try await finalize(parsed, .geminiNotes, raw: trimmed,
                                      shouldGenerateTitle: generateTitleIfMissing, titleHint: titleHint, dateHint: dateHint)
        case .subtitle:
            return try await finalize(SubtitleParser.parse(trimmed), .subtitle, raw: trimmed,
                                      shouldGenerateTitle: generateTitleIfMissing, titleHint: titleHint, dateHint: dateHint)
        case .unknown:
            // Local-only: keep the raw text on-device instead of shipping it to the cloud LLM to structure.
            var p = allowCloud ? try await aiResolve(trimmed) : Self.deterministicParse(trimmed)
            if let titleHint, p.title == nil || p.title == "Imported call" { p.title = titleHint }
            if let dateHint, p.date == nil { p.date = dateHint }
            return Resolved(transcript: p, format: .unknown, usedAI: allowCloud)
        }
    }

    /// Local-only fallback for an unrecognized format: one utterance per non-empty line, no speaker, no
    /// cloud call — content is preserved on-device (SpeakerResolver renumbers the placeholder at display).
    static func deterministicParse(_ raw: String) -> ParsedTranscript {
        let lines = raw.split(separator: "\n").map { $0.trimmingCharacters(in: .whitespaces) }.filter { !$0.isEmpty }
        let utterances = lines.enumerated().map { i, line in
            ParsedUtterance(seq: i, speakerRaw: "Unknown", speakerConfidence: 1.0,
                            tStart: 0, tEnd: 0, text: line, isInferredSpeaker: false, tsConfidence: .none)
        }
        return ParsedTranscript(title: nil, date: nil, startedAt: nil, durationSeconds: nil,
                                source: .paste, speakers: [], utterances: utterances)
    }
    ...
    private func finalize(_ parsed: ParsedTranscript, _ fmt: Format, raw: String,
        ...
        if shouldGenerateTitle, (p.title?.isEmpty ?? true) {
            p.title = allowCloud
                ? ((try? await generateTitle(forSpeakers: p.speakers, sample: raw)) ?? defaultTitle(p))
                : defaultTitle(p)   // local-only: no cloud title generation
        }
