...
///   • `Travis  0:00`            (name, then a timecode at end of line)
///   • `Travis (0:00): text…`    (name, parenthesized timecode, optional inline text)
...
public enum FathomParser {
    ...
    // Compile once (these literals are valid, so `try!` is safe). NSRegularExpression is documented
    // thread-safe for matching but isn't Sendable-marked → nonisolated(unsafe) under Swift 6.
    private nonisolated(unsafe) static let inlineRE = try! NSRegularExpression(
        pattern: #"^\s*(.+?)\s*\((\d{1,2}:\d{2}(?::\d{2})?)\)\s*:?\s*(.*)$"#)
    private nonisolated(unsafe) static let endRE = try! NSRegularExpression(
