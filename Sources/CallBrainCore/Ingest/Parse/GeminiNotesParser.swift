...
public enum GeminiNotesParser {
    /// The pseudo-speaker every Gemini-notes line is stored under. Consumers use this to keep notes OUT of
    /// the verbatim "who said what" transcript (e.g. after a notes↔recording merge) — notes are a summary,
    /// not dialogue. Shared so the parser and the UI can never drift on the exact string.
    public static let pseudoSpeaker = "Gemini Notes"
    ...
    public static func parse(_ text: String, title: String? = nil, date: String? = nil) throws -> ParsedTranscript {
        ...
        let utterances = lines.enumerated().map { i, line in
            ParsedUtterance(seq: i, speakerRaw: pseudoSpeaker, speakerConfidence: nil,
        ...
        return ParsedTranscript(title: title, date: date, startedAt: nil, durationSeconds: nil,
                                source: .gmeetGemini, speakers: [pseudoSpeaker], utterances: utterances)
    }
