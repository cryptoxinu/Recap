...
struct ImportView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("Drop a Fathom / Fireflies / Cluely / Google-Meet export (or any transcript) below — "
                 + "Recap detects the format, structures it, names it, and indexes it. "
    ...
    private func chooseFolder() {
        ...
        panel.prompt = "Import"
        panel.message = "Choose a folder of transcripts and recordings — Recap imports them all."
        if panel.runModal() == .OK, let folder = panel.url {
            Task {
                let n = await coordinator.enqueueFolder(folder)
                // Don't mask a real enqueue/persistence error with the generic empty message — only
                // claim "nothing found" when the queue itself reported no failure (audit G3 MED).
                if n == 0, coordinator.lastError == nil {
                    coordinator.lastError = "No importable transcripts or recordings found in that folder."
                }
            }
        }
    }
            ...
            if showPaste {
                ...
                HStack {
                    Button {
                        ...
                        Label("Import pasted text", systemImage: "doc.on.clipboard")
                    }
    ...
    private var queueSummary: String {
        ...
        let dups = jobs.filter { $0.message?.hasPrefix("Already imported") == true }.count
        var s = "\(done) of \(jobs.count)"
        if dups > 0 { s += " · \(dups) duplicate\(dups == 1 ? "" : "s") skipped" }
            ...
            VStack(alignment: .leading, spacing: 10) {
                HStack {
                    ...
                    Button("Clear finished") { Task { await coordinator.clearFinished() } }
                        ...
                        // Only clears done/failed jobs — disable when there are none, so it isn't a
                        // button that looks live but does nothing (audit G3 LOW).
                        .disabled(!coordinator.jobs.contains { $0.state == .done || $0.state == .failed })
                }
                VStack(spacing: 0) {
                    ForEach(coordinator.jobs) { job in
                        JobRow(job: job,
                               onOpen: { if let id = job.meetingID { env.openMeeting(id) } },
                               onConfirm: { Task { await coordinator.confirmReviewed(job) } },
                               onRemove: { Task { await coordinator.remove(job) } },
                               onRetry: { Task { await coordinator.retry(job) } })
...
private struct JobRow: View {
    ...
    @ViewBuilder private var actions: some View {
        HStack(spacing: 8) {
            ...
            if job.state == .failed, let onRetry {
                Button("Retry") { onRetry() }.buttonStyle(.bordered).controlSize(.small)
            }
