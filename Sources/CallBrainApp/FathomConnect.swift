...
final class FathomConnect {
    ...
    /// Max the sync watermark is held back for a still-processing meeting — beyond this a
    /// never-transcribed call is given up on so it can't freeze the watermark forever (audit D5).
    private static let maxHoldback: TimeInterval = 14 * 86_400
    ...
    init(env: AppEnvironment) {
        ...
        // Pull the moment you bring Recap to the front — so right after a meeting, opening the app
    ...
    func syncNow() async {
        ...
        do {
            ...
            var imported = 0
            var pendingTimes: [Date] = []   // createdAt of still-processing (empty-transcript) meetings
            for m in page.meetings.sorted(by: { ($0.createdAt ?? .distantPast) < ($1.createdAt ?? .distantPast) }) {
                guard live() else { break }
                if m.lines.isEmpty {                                   // transcript not ready
                    if let t = m.createdAt { pendingTimes.append(t) }  // hold the watermark for it (audit D5)
                    continue
                }
            ...
            if page.complete {
                ...
                // Hold the forward watermark at the EARLIEST still-processing meeting so its
                // transcript is still inside the lookback window next poll — else a transcript that
                // takes longer than `window` to appear is lost forever once the watermark passes it
                // (audit D5). Bounded by `maxHoldback` so a never-transcribed call can't freeze the
                // watermark permanently. Stays forward-only (never below the prior lastSync).
                var target = startTime
                if let earliest = pendingTimes.min() {
                    target = min(target, max(earliest, startTime.addingTimeInterval(-Self.maxHoldback)))
                }
                let advanced = max(creds.lastSync ?? .distantPast, target)
