...
final class ChatModel {
    ...
    func ask(_ text: String, _ env: AppEnvironment) async {
        ...
        // Ensure a conversation exists (auto-titled from the first question).
        ...
        persist(.user, text: q, citations: [], conversationID: convID, env)
        ...
        do {
            ...
            persist(.assistant, text: ans.text, citations: stored, conversationID: convID, env)
            refreshRecents(env)
        } catch {
            if let i = messages.firstIndex(where: { $0.id == pid }) {
