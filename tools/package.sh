#!/bin/bash
# Recap — Developer-ID sign + notarize + DMG (Phase 8 packaging).
#
# Produces a signed, notarized, stapled Recap.app + a .dmg for direct download (NOT the App Store —
...
APP="$ROOT/.build/Recap.app"
DMG="$ROOT/.build/Recap.dmg"
ENT="$ROOT/tools/Recap.entitlements"
...
codesign --force --options runtime --timestamp --sign "$SIGN_ID" "$APP/Contents/MacOS/cbtranscribe"
codesign --force --options runtime --timestamp --sign "$SIGN_ID" "$APP/Contents/MacOS/cbpairhost"
codesign --force --options runtime --timestamp --entitlements "$ENT" --sign "$SIGN_ID" "$APP"
codesign --verify --deep --strict --verbose=2 "$APP"

echo "▸ DMG"
rm -f "$DMG"
hdiutil create -volname "Recap" -srcfolder "$APP" -ov -format UDZO "$DMG"
